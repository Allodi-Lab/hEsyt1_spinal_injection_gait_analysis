...
setwd("~/Documents/ESYT1_spinal_locomotion_analysis/Lateral video analysis/") #Change the path to where you have the 'Timepoint analysis' map located
df<- read.table("Statistics_lateral_updated.csv", sep="\t", header=T)
df_19E<- read.table("statistics_p56_p63_19E.csv", sep="\t", header=T) #change 'belt speed' to belt.speed
...
df2$Genotype <- ifelse(grepl('_19_', df$name), 'SOD1En1cre', df2$Genotype)
df2$Genotype <- ifelse(grepl('_50_', df$name), 'SOD1En1cre', df2$Genotype)
df2$Genotype <- ifelse(grepl('_237_', df$name), 'SOD1En1cre', df2$Genotype)
df2$Genotype <- ifelse(grepl('_239_', df$name), 'SOD1En1cre', df2$Genotype)
df2$Genotype <- ifelse(grepl('_318_', df$name), 'SOD1En1cre', df2$Genotype)
df2$Genotype <- ifelse(grepl('_44_', df$name), 'SOD1En1cre', df2$Genotype)

#Convert Genotype to En1cre
df2$Genotype <- ifelse(grepl('_9_', df$name), 'En1cre', df2$Genotype)
df2$Genotype <- ifelse(grepl('_19E_', df$name), 'En1cre', df2$Genotype)
df2$Genotype <- ifelse(grepl('_33_', df$name), 'En1cre', df2$Genotype)
df2$Genotype <- ifelse(grepl('_80_', df$name), 'En1cre', df2$Genotype)
df2$Genotype <- ifelse(grepl('_42_', df$name), 'En1cre', df2$Genotype)
df2$Genotype <- ifelse(grepl('_48_', df$name), 'En1cre', df2$Genotype)
df2$Genotype <- ifelse(grepl('_62_', df$name), 'En1cre', df2$Genotype)
df2$Genotype <- ifelse(grepl('_66_', df$name), 'En1cre', df2$Genotype)

#Convert Genotype to SOD1
df2$Genotype <- ifelse(grepl('_17_', df$name), 'SOD1', df2$Genotype)
df2$Genotype <- ifelse(grepl('_21_', df$name), 'SOD1', df2$Genotype)
df2$Genotype <- ifelse(grepl('_52_', df$name), 'SOD1', df2$Genotype)
df2$Genotype <- ifelse(grepl('_56_', df$name), 'SOD1', df2$Genotype)
df2$Genotype <- ifelse(grepl('_233_', df$name), 'SOD1', df2$Genotype)
df2$Genotype <- ifelse(grepl('_235_', df$name), 'SOD1', df2$Genotype)
df2$Genotype <- ifelse(grepl('_320_', df$name), 'SOD1', df2$Genotype)
df2$Genotype <- ifelse(grepl('_1_', df$name), 'SOD1', df2$Genotype)
df2$Genotype <- ifelse(grepl('_7_', df$name), 'SOD1', df2$Genotype)
df2$Genotype <- ifelse(grepl('_40_', df$name), 'SOD1', df2$Genotype)

#Convert Genotype to wt
df2$Genotype <- ifelse(grepl('_11_', df$name), 'wt', df2$Genotype)
df2$Genotype <- ifelse(grepl('_13_', df$name), 'wt', df2$Genotype)
df2$Genotype <- ifelse(grepl('_15_', df$name), 'wt', df2$Genotype)
df2$Genotype <- ifelse(grepl('_54_', df$name), 'wt', df2$Genotype)
df2$Genotype <- ifelse(grepl('_316_', df$name), 'wt', df2$Genotype)
df2$Genotype <- ifelse(grepl('_3_', df$name), 'wt', df2$Genotype)
df2$Genotype <- ifelse(grepl('_5_', df$name), 'wt', df2$Genotype)
df2$Genotype <- ifelse(grepl('_46_', df$name), 'wt', df2$Genotype)
```
...
#SOD1En1cre
df2$mouse_number <- ifelse(grepl('_19_', df$name), '19', df2$mouse_number)
df2$mouse_number <- ifelse(grepl('_50_', df$name), '50', df2$mouse_number)
df2$mouse_number <- ifelse(grepl('_237_', df$name), '237', df2$mouse_number)
df2$mouse_number <- ifelse(grepl('_239_', df$name), '239', df2$mouse_number)
df2$mouse_number <- ifelse(grepl('_318_', df$name), '318', df2$mouse_number)
df2$mouse_number <- ifelse(grepl('_44_', df$name), '44', df2$mouse_number)

#En1re
df2$mouse_number <- ifelse(grepl('_9_', df$name), '9', df2$mouse_number)
df2$mouse_number <- ifelse(grepl('_19E_', df$name), '19E', df2$mouse_number) 
df2$mouse_number <- ifelse(grepl('_33_', df$name), '33', df2$mouse_number)
df2$mouse_number <- ifelse(grepl('_80_', df$name), '80', df2$mouse_number)
df2$mouse_number <- ifelse(grepl('_42_', df$name), '42', df2$mouse_number)
df2$mouse_number <- ifelse(grepl('_48_', df$name), '48', df2$mouse_number)
df2$mouse_number <- ifelse(grepl('_62_', df$name), '62', df2$mouse_number)
df2$mouse_number <- ifelse(grepl('_66_', df$name), '66', df2$mouse_number)

#SOD1
df2$mouse_number <- ifelse(grepl('_17_', df$name), '17', df2$mouse_number)
df2$mouse_number <- ifelse(grepl('_21_', df$name), '21', df2$mouse_number)
df2$mouse_number <- ifelse(grepl('_52_', df$name), '52', df2$mouse_number)
df2$mouse_number <- ifelse(grepl('_56_', df$name), '56', df2$mouse_number)
df2$mouse_number <- ifelse(grepl('_233_', df$name), '233', df2$mouse_number)
df2$mouse_number <- ifelse(grepl('_235_', df$name), '235', df2$mouse_number)
df2$mouse_number <- ifelse(grepl('_320_', df$name), '320', df2$mouse_number)
df2$mouse_number <- ifelse(grepl('_1_', df$name), '1', df2$mouse_number)
df2$mouse_number <- ifelse(grepl('_7_', df$name), '7', df2$mouse_number)
df2$mouse_number <- ifelse(grepl('_40_', df$name), '40', df2$mouse_number)

#wt
df2$mouse_number <- ifelse(grepl('_11_', df$name), '11', df2$mouse_number)
df2$mouse_number <- ifelse(grepl('_13_', df$name), '13', df2$mouse_number)
df2$mouse_number <- ifelse(grepl('_15_', df$name), '15', df2$mouse_number)
df2$mouse_number <- ifelse(grepl('_54_', df$name), '54', df2$mouse_number)
df2$mouse_number <- ifelse(grepl('_316_', df$name), '316', df2$mouse_number)
df2$mouse_number <- ifelse(grepl('_3_', df$name), '3', df2$mouse_number)
df2$mouse_number <- ifelse(grepl('_5_', df$name), '5', df2$mouse_number)
df2$mouse_number <- ifelse(grepl('_46_', df$name), '46', df2$mouse_number)
```
